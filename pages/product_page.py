...
class ProductPage(BasePage):
    ...
    def confirm_add_book_name(self):
        assert self.is_element_present(
            *BasketPageLocators.CONFIRM_BOOK_NAME), "the name of the book added in basket does not match"

    def confirm_add_book_price(self):
        assert self.is_element_present(
            *BasketPageLocators.CONFIRM_BOOK_PRICE), "the price of the book added in basket does not match"
    ...
    def confirm_add_book_name__param(self):
        ...
        assert self.browser.find_element_by_xpath(
            f"//strong[text()='{name}']"), "the name of the book added in basket does not match _ 2"

    def confirm_add_book_price_param(self):
        price_in_title = self.search_book_price()
        try:
            price_in_basket = self.browser.find_element(*ProductPageLocators.MINI_BASKET).text
    ...
    def disappeared_success_message(self):
        assert self.is_disappeared(
            *ProductPageLocators.SUCCESS_MESSAGE), "Success message is not disappeared, and it shouldn't"
